...
def prepared(fake):
    ...
    doctor = random.choice(name[20:37])
    doctor2 = random.choice(name[20:37])
    if gender == "Male":
        customer = {
    ...
    return (customer, description, birthday, arrive_time,
            ...
            paramedic, paramedic2, nurse, doctor, doctor2)
...
def fake_data_gen(length, locale, seed) -> dict:
    ...
    for ix in range(1, length + 1):
        ...
        id_hospital, brigade_code, paramedic, paramedic2, nurse, doctor, doctor2 = prepared(fake)
        yield {
            "call_id": ix,
            **customer,
            ...
            "patient_code": random.randint(1, length + 1),
...
def to_local_file(file_out: str, fields_dict: List, length: int = 10000, locale: str = "en_US", seed: int = 42):
    ...
    """
    with open(f"{str(Path(__file__).parent.parent)}/data_files/{file_out}.csv", 'w', newline='',
...
def load_to_aws(file_out: str, bucket: str):
    session = boto3.session.Session()
    client = session.client("s3")
    with open(f"{str(Path(__file__).parent.parent)}/data_files/{file_out}.csv", 'rb') as csvfile:
        client.put_object(
            Bucket=bucket,
            Key=f"data/{file_out}.csv",
            Body=csvfile
        )
...
if __name__ == "__main__":
    fields = ["call_id", "first_name", "last_name", "address", "date_of_birth", "description",
              "arrive_time", "patient_code", "insurance_types", "id_hospital", "brigade_code",
              "paramedic", "paramedic2", "nurse", "doctor", "doctor2"]
    to_local_file("out", fields, length=100000, locale='en_US', seed=42)
    load_to_aws("out", "aws-warehouse")
